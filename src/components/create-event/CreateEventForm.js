...
class CreateEventForm extends Component {
	constructor(props) {
		...
		this.state = {
			event: {
				...
				endDate: '',
				endTime: '',
				vendorName: '', //NEWLY ADDED
				vendorAddress: '', //NEWLY ADDED
				vendorStatus: '', //NEWLY ADDED
				vendorServices: '', //NEWLY ADDED
				guests: []
			},
			errors: [],
			focusGuests: false,
	...
	onSubmitForm(evt) {
		...
		const vendorName = this.state.event.vendorName; //NEWLY ADDED
		const vendorAddress = this.state.event.vendorAddress; //NEWLY ADDED
		const vendorStatus = this.state.event.vendorStatus; //NEWLY ADDED
		const vendorServices = this.state.event.vendorServices; //NEWLY ADDED
		...
		if (!vendorName) errors.push({ type: 'vendorName', msg: 'Please provide name of vendor' }); //NEWLY ADDED
		if (!vendorAddress) errors.push({ type: 'vendorAddress', msg: 'Please provide vendor address' }) //NEWLY ADDED
		if (!vendorStatus) errors.push({ type: 'vendorStatus', msg: 'Please provide vendor status' }) //NEWLY ADDED
		if (!vendorServices) errors.push({ type: 'vendorServices', msg: 'Please enter desired vendor services' }) //NEWLY ADDED
		...
		const newEvent = {
			name, type, host, startDate, startTime, endDate, endTime, location, message, guests, vendorName, vendorAddress, vendorStatus, vendorServices
			//NEWLY MODIFIED
		};
		...
		return (
			<div className="row">
				...
				<form onKeyPress={this.onFormKeyPress} ref={(eventForm) => { this.eventForm = eventForm; }} onSubmit={this.onSubmitForm} className="col s12">
					...
					<TextField //NEWLY ADDED STARTS HERE
						onInput={this.onChangeTextField}
						value={this.state.event.vendorName}
						placeholder="Type in the vendor name"
						label="Vendor Name"
						id="vendorName"
						errors={this.state.errors}
					/>
					...
					<TextField //NEWLY ADDED STARTS HERE
						onInput={this.onChangeTextField}
						value={this.state.event.vendorAddress}
						placeholder="Type in the vendor address"
					...
					<TextField //NEWLY ADDED STARTS HERE
						onInput={this.onChangeTextField}
						value={this.state.event.vendorStatus}
						placeholder="Type in the vendor status"
						label="Vendor Status"
						id="vendorStatus"
						errors={this.state.errors}
					/>
					...
					<TextField //NEWLY ADDED STARTS HERE
						onInput={this.onChangeTextField}
						value={this.state.event.vendorServices}
